  ...
  const panelContent: Record<string, JSX.Element> = {
    chart: (
      <div>
        ...
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>Multi-timeframe candlestick charts with on-chain overlays. MVRV, realized cap, price/200d — understand cycle position, not just price.</p>
      </div>
    ),
    indicators: (
      <div>
        ...
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>Interest rates, inflation, GDP, employment, yield curve — all structured and live with directional context.</p>
      </div>
    ),
    watchlist: (
      <div>
        ...
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>Your watchlist connected to filtered news, signals, and contextual analysis. Not a static price list.</p>
      </div>
    ),
    news: (
      <div>
        ...
        {newsItems.slice(0, 4).map((n, i) => (
          <div key={i} className="flex justify-between py-2 border-b last:border-0 text-sm" style={{ borderColor: 'var(--terminal-border)' }}>
            ...
            <span className="text-sm font-mono shrink-0 ml-3" style={{ color: 'var(--t4)' }}>{n.time}</span>
          </div>
        ))}
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>Curated news filtered by watchlist with importance badges and asset tagging.</p>
      </div>
    ),
    cot: (
      <div>
        ...
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>CFTC institutional positioning. Smart money vs spec. Crowded positioning alerts.</p>
      </div>
    ),
    risk: (
      <div>
        ...
        <p className="mt-4 text-sm" style={{ color: 'var(--t4)' }}>VIX regime, credit spreads, breadth, liquidity, dollar strength — the complete risk dashboard.</p>
      </div>
    ),
  }
  ...
  return (
    <section className="py-20" id="pricing">
      <div className="max-w-site mx-auto px-8">
        <div className="text-center mb-10">
          ...
          <div className="inline-flex rounded-lg p-0.5 mt-6" style={{ border: '1px solid var(--border)', background: 'var(--surface)' }}>
            <button onClick={() => setYearly(false)}
              className={cn('px-4 py-1.5 rounded-md text-sm font-semibold transition-all', !yearly && 'text-white')}
              style={{ background: !yearly ? 'var(--bh)' : 'transparent', color: yearly ? 'var(--t3)' : undefined }}>
              Monthly
            </button>
            <button onClick={() => setYearly(true)}
              className={cn('px-4 py-1.5 rounded-md text-sm font-semibold transition-all flex items-center gap-1.5', yearly && 'text-white')}
              style={{ background: yearly ? 'var(--bh)' : 'transparent', color: !yearly ? 'var(--t3)' : undefined }}>
              Yearly <span className="text-sm px-1.5 py-0.5 rounded font-bold" style={{ color: 'var(--acc)', background: 'var(--acc-d)' }}>-33%</span>
            </button>
          </div>
        </div>
        <div className="grid md:grid-cols-3 gap-3 max-w-[960px] mx-auto">
          {plans.map(plan => (
            <div key={plan.name} className={cn('relative p-6 rounded-xl transition-all', plan.popular && 'ring-1')}
              ...
              <p className="text-sm mb-4" style={{ color: 'var(--t3)' }}>{plan.desc}</p>
              <div className="text-[2rem] font-bold tracking-tight mb-0.5" style={{ letterSpacing: '-0.02em' }}>
                ${yearly ? plan.priceY : plan.priceM}
                {plan.priceM > 0 && <span className="text-sm font-normal" style={{ color: 'var(--t3)' }}>/mo</span>}
              </div>
              <div className="text-sm mb-5" style={{ color: 'var(--t4)' }}>
              ...
              <Link href={plan.priceM === 0 ? '/signup' : '/signup'}
                className={cn('w-full py-2.5 rounded-lg text-sm font-semibold transition-all active:scale-[.98] text-center block',
        ...
        <p className="text-center text-sm mt-6" style={{ color: 'var(--t4)' }}>Prices exclude applicable taxes. Cancel anytime.</p>
      </div>
    </section>
  )
}
      ...
      <div className="max-w-site mx-auto px-8">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-16">
          <div className="col-span-2">
            ...
            <p className="text-sm leading-relaxed max-w-[260px]" style={{ color: 'var(--t2)' }}>
        ...
        >
          ...
          <p className="text-sm max-w-xl md:text-right leading-relaxed" style={{ color: 'var(--t3)' }}>

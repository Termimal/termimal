...
export function MarketsSection() {
  ...
  return (<section ref={sectionRef as React.Ref<HTMLElement>} className="py-20" id="markets"><div className="max-w-site mx-auto px-8"><div className="mb-8 reveal"><div className="section-label">Market coverage</div><div className="section-title">Every market. One workspace.</div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-3">{marketCards.map((m, i) => (<div key={m.name} className="p-5 rounded-xl cursor-pointer transition-all hover:-translate-y-1 hover:shadow-lg reveal" style={{ border: '1px solid var(--border)', background: 'var(--surface)', transitionDelay: `${i * 70}ms` }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = 'rgba(52,211,153,.25)'; e.currentTarget.style.boxShadow = '0 8px 32px rgba(52,211,153,.06)' }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = 'var(--border)'; e.currentTarget.style.boxShadow = 'none' }}><div className="text-2xl font-bold mb-0.5" style={{ color: 'var(--acc)' }}>{m.count}</div><div className="text-sm font-semibold mb-0.5">{m.name}</div><div className="text-sm" style={{ color: 'var(--t3)' }}>{m.desc}</div></div>))}</div></div></section>)
}

export function PricingSection() {
  const [yearly, setYearly] = useState(true)
  const sectionRef = useReveal(0.08)
  return (<section ref={sectionRef as React.Ref<HTMLElement>} className="py-20" id="pricing"><div className="max-w-site mx-auto px-8"><div className="text-center mb-10 reveal"><div className="section-label">Pricing</div><div className="section-title">Transparent pricing. No surprises.</div><p className="text-sm max-w-sm mx-auto mt-1" style={{ color: 'var(--t3)' }}>Start free. Upgrade when ready. Cancel anytime.</p><div className="inline-flex rounded-lg p-0.5 mt-6" style={{ border: '1px solid var(--border)', background: 'var(--surface)' }}><button onClick={() => setYearly(false)} className={cn('px-4 py-1.5 rounded-md text-sm font-semibold transition-all', !yearly && 'text-white')} style={{ background: !yearly ? 'var(--bh)' : 'transparent', color: yearly ? 'var(--t3)' : undefined }}>Monthly</button><button onClick={() => setYearly(true)} className={cn('px-4 py-1.5 rounded-md text-sm font-semibold transition-all flex items-center gap-1.5', yearly && 'text-white')} style={{ background: yearly ? 'var(--bh)' : 'transparent', color: !yearly ? 'var(--t3)' : undefined }}>Yearly <span className="text-sm px-1.5 py-0.5 rounded font-bold" style={{ color: 'var(--acc)', background: 'var(--acc-d)' }}>-33%</span></button></div></div><div className="grid md:grid-cols-3 gap-3 max-w-[960px] mx-auto">{plans.map((plan, i) => (<div key={plan.name} className={cn('relative p-6 rounded-xl transition-all reveal', plan.popular && 'ring-1')} style={{ border: plan.popular ? '1px solid rgba(52,211,153,.25)' : '1px solid var(--border)', background: plan.popular ? 'var(--acc-d)' : 'var(--surface)', transitionDelay: `${i * 90}ms`, ...(plan.popular ? { animation: 'borderGlow 3s ease-in-out infinite' } : {}) }}>{plan.popular && (<div className="absolute -top-2.5 left-5 px-2.5 py-0.5 rounded-md text-sm font-bold uppercase tracking-wide text-white" style={{ background: 'var(--acc2)' }}>Popular</div>)}<h3 className="text-base font-bold mb-0.5">{plan.name}</h3><p className="text-sm mb-4" style={{ color: 'var(--t3)' }}>{plan.desc}</p><div className="text-[2rem] font-bold tracking-tight mb-0.5" style={{ letterSpacing: '-0.02em' }}>${yearly ? plan.priceY : plan.priceM}{plan.priceM > 0 && <span className="text-sm font-normal" style={{ color: 'var(--t3)' }}>/mo</span>}</div><div className="text-sm mb-5" style={{ color: 'var(--t4)' }}>{plan.priceM === 0 ? '14-day trial' : yearly ? 'Billed annually' : 'Billed monthly'}</div><ul className="flex flex-col gap-2 mb-6">{plan.features.map(f => (<li key={f} className="flex items-center gap-2 text-sm" style={{ color: 'var(--t3)' }}><span className="text-sm" style={{ color: 'var(--acc)' }}>&#10003;</span> {f}</li>))}</ul><Link href={plan.priceM === 0 ? '/signup' : '/signup'} className={cn('w-full py-2.5 rounded-lg text-sm font-semibold transition-all active:scale-[.98] text-center block', plan.popular ? 'text-white hover:brightness-110' : 'hover:border-[var(--bh)]')} style={{ background: plan.popular ? 'var(--acc2)' : 'transparent', border: plan.popular ? 'none' : '1px solid var(--border)', color: plan.popular ? '#fff' : 'var(--t2)' }}>{plan.cta}</Link></div>))}</div><p className="text-center text-sm mt-6" style={{ color: 'var(--t4)' }}>Prices exclude applicable taxes. Cancel anytime.</p></div></section>)
}
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-8 mb-16">
          ...
          {cols.map(col => (
            <div key={col.title}>
              ...
              <ul className="flex flex-col gap-2.5">{col.links.map(link => (<li key={link}><Link href={footerLinkUrl(link)} className="text-sm transition-all hover:text-white" style={{ color: 'var(--t3)' }}>{link}</Link></li>))}</ul>
            </div>
          ))}
        </div>

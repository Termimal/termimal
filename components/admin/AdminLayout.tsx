    ...
    <div className="min-h-screen flex" style={{ background: 'var(--bg)' }}>
      <aside className="w-52 shrink-0 p-3 flex flex-col border-r" style={{ borderColor: 'var(--border)', background: 'var(--surface)' }}>
        <div className="flex items-center gap-2 mb-6 px-2 pt-1">
          ...
          <span className="text-sm font-bold px-1.5 py-0.5 rounded ml-auto" style={{ color: 'var(--red-val)', background: 'rgba(248,113,113,.1)' }}>ADMIN</span>
        </div>
          ...
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href}
                className={cn('flex items-center gap-2 px-2.5 py-1.5 rounded-md text-sm font-medium transition-all')}
        ...
        <div className="flex items-center gap-2 mt-auto pt-3 border-t" style={{ borderColor: 'var(--border)' }}>
          <ThemeToggle />
          <Link href="/" className="text-sm font-medium" style={{ color: 'var(--t4)' }}>← Back to site</Link>
        </div>
      </aside>
      <main className="flex-1 overflow-auto">
        <header className="flex items-center justify-between px-6 py-3 border-b" style={{ borderColor: 'var(--border)' }}>
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm font-mono" style={{ color: 'var(--t4)' }}>Super Admin</span>
            <div className="w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold" style={{ background: 'rgba(248,113,113,.1)', color: 'var(--red-val)' }}>SA</div>
          </div>
        </header>

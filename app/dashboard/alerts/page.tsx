          ...
          <table className="w-full text-xs">
            ...
            <tbody>{alerts.map(a => (
              <tr key={a.id} style={{ borderTop: '1px solid var(--border)' }}>
                ...
                <td className="p-4"><span className="px-2 py-0.5 rounded text-[0.6rem] font-semibold" style={{ color: a.status === 'active' ? 'var(--green-val)' : 'var(--amber)', background: a.status === 'active' ? 'rgba(52,211,153,.1)' : 'rgba(251,191,36,.1)' }}>{a.status}</span></td>
              </tr>
            ))}</tbody>
          </table>
        </div>
